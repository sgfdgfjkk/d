...
// flip to false to re-enable case battles
const CASE_BATTLES_DISABLED = true;
...
const Api = {
  ...
  async addWager(name, amount) {
    try {
      const r = await fetch('/api/wager', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, amount }) });
      if (!r.ok) return null;
...
const CASE_IMG = {
  ...
  relic: { img: 'cases/case-relic.png', c: '#c9a06c' },
};
...
function renderCbCases() {
  ...
  const qb = document.getElementById('acQuickbar'); // removed from UI
  if (false && qb) {
    const prices = Object.values(CASE_TYPES).map((c) => c.price);
    const best = Math.max.apply(null, prices);
    qb.innerHTML =
      '<span class="ac-chip"><b>' + Object.keys(CASE_TYPES).length + '</b> cases</span>' +
      '<span class="ac-chip"><b>' + fmt(Math.min.apply(null, prices)) + '</b> cheapest</span>' +
      '<span class="ac-chip gold"><b>' + fmt(best) + '</b> top case</span>' +
      '<span class="ac-chip"><b>' + (currentUser ? fmt(currentUser.balance) : '—') + '</b> your balance</span>';
  }
...
function renderPv() {
  ...
  let cost = selCases.reduce((s, k) => s + CASE_TYPES[k].price, 0);
...
async function createBattle() {
  ...
  trackWager(currentUser.name, selCases.reduce((s, k) => s + CASE_TYPES[k].price, 0));
    setBalance(currentUser.balance - cost);
  try {
    const sb = await Api.createBattle({ mode: selMode, type: selType, borrow: borrowOn ? borrowPct : 0, cases: [...selCases], name: currentUser.name });
...
/* ---------- router ---------- */
function renderLeaderboard(serverRows) {
  const rows = (serverRows && serverRows.length ? serverRows : (window.__lbTop || [])).map(function (r) {
    return { n: r.name, lvl: botLvl(r.name), k: avatarFor(r.name), wagered: r.wagered || 0 };
  });
  const prizes = [100000, 100000, 70000, 50000, 35000, 28000, 22000, 18000, 15000, 12000];
  if (!rows.length) {
    const pod = document.getElementById('lbPodium');
    const tab = document.getElementById('lbTable');
    if (pod) pod.innerHTML = '<div class="lb-empty">No wagers yet this week — be the first on the board!</div>';
    if (tab) tab.innerHTML = '<div class="lb-empty">Play a battle to enter the race.</div>';
    return;
  }
  const pod = document.getElementById('lbPodium');
  if (pod) {
    ...
    const items = ['models/robux.glb', 'models/magic3d_pbr_model_3c4622a2-0146-42d0-85f5-f58a3edb8e4b.glb', 'models/magic3d_pbr_model_39b24bbc-b7a5-4781-be29-787002a133a5.glb'];
    pod.innerHTML = order.map(function (i) {
      const r = rows[i];
      const crown = i === 0 ? '<svg class="pod-crown" viewBox="0 0 24 24" width="34" height="34"><path fill="#5ea0ff" d="M3 8l4.5 4L12 5l4.5 7L21 8l-1.6 10H4.6L3 8zm2 12h14v2H5v-2z"/></svg>' : '';
      const inner = r ? avatarSVG(r.k) : '<span class="pod-q">?</span>';
      return '<div class="pod-card ' + colors[i] + (r ? '' : ' blank') + '">' +
        '<span class="pod-place">' + (i + 1) + '<em>' + (i === 0 ? 'st' : i === 1 ? 'nd' : 'rd') + '</em> PLACE</span>' +
        '<div class="pod-hexwrap">' + crown + '<div class="pod-hex ' + colors[i] + '">' + inner + '</div></div>' +
        '<model-viewer class="pod-item" src="' + items[i] + '" alt="" auto-rotate rotation-per-second="14deg"' +
          ' camera-orbit="0deg 72deg 105%" field-of-view="26deg" interaction-prompt="none"' +
          ' shadow-intensity="0" exposure="1.15"></model-viewer>' +
        '<b class="pod-name">' + (r ? r.n : '—') + '</b>' +
        '<span class="pod-wagered">WAGERED <svg viewBox="0 0 24 24" width="12" height="12"><use href="#coin"/></svg> ' + (r ? fmt(r.wagered) : '0.00') + '</span>' +
        '<div class="pod-ribbon"><span>PRIZE</span><svg viewBox="0 0 24 24" width="13" height="13"><use href="#coin"/></svg><b>' + fmt(prizes[i]) + '</b></div>' +
        '</div>';
    }).join('');
  }
  const table = document.getElementById('lbTable');
  if (table) {
    table.innerHTML = '<div class="lb-thead"><span>Place</span><span>User</span><span>Wagered</span><span>Prize</span></div>' +
      Array.from({ length: 10 }, function (_, i) { const r = rows[i];
        const medal = i < 3 ? '<svg class="lb-trophy" viewBox="0 0 24 24" width="15" height="15"><path fill="' + ['#ffd35c', '#c8d2e8', '#e8965a'][i] + '" d="M6 3h12v2h3v3c0 2.5-2 4.5-4.4 4.9A6 6 0 0 1 13 16.9V19h3v2H8v-2h3v-2.1a6 6 0 0 1-3.6-3.9C5 12.5 3 10.5 3 8V5h3V3zm-1 4v1c0 1.2.8 2.3 2 2.7V7H5zm14 0h-2v3.7c1.2-.4 2-1.5 2-2.7V7z"/></svg>' : '';
        if (!r) {
        ...
        return '<div class="lb-row' + (i < 3 ? ' top' : '') + '">' +
          ...
          '<span class="lb-wagered"><svg viewBox="0 0 24 24" width="12" height="12"><use href="#coin"/></svg>' + fmt(r.wagered) + '</span>' +
          '<span class="lb-prize"><svg viewBox="0 0 24 24" width="12" height="12"><use href="#coin"/></svg>' + fmt(prizes[i] || 0) + '</span>' +
          '</div>';
      }).join('');
  }
  // weekly countdown (resets Sunday midnight)
  const end = (() => { const d = new Date(); d.setDate(d.getDate() + (7 - d.getDay()) % 7 || 7); d.setHours(0, 0, 0, 0); return d.getTime(); })();
  const upd = () => {
    let s = Math.max(0, Math.floor((end - Date.now()) / 1000));
    const d = Math.floor(s / 86400); s -= d * 86400;
    const hh = Math.floor(s / 3600); s -= hh * 3600;
    const mm = Math.floor(s / 60); s -= mm * 60;
    const set = (id2, v) => { const el = document.getElementById(id2); if (el) el.textContent = v; };
    set('lbD', d + 'd'); set('lbH', hh + 'h'); set('lbM', mm + 'm'); set('lbS', s + 's');
  };
  upd();
  window.__lbTimer && clearInterval(window.__lbTimer);
  window.__lbTimer = setInterval(upd, 1000);
}
...
function renderRoute() {
  ...
  $('#view-leaderboard').hidden = h !== '#/leaderboard';
  ...
  if (h === '#/leaderboard') Api.getLeaderboard().then(function (top) { renderLeaderboard(top); });
  if (h === '#/leaderboard') {
    const wrapEl = document.querySelector('.lb-wrap');
    if (wrapEl && !wrapEl.querySelector('.lb-asteroids')) {
      const field = document.createElement('div');
      field.className = 'lb-asteroids';
      for (let i = 0; i < 7; i++) {
        const a = document.createElement('i');
        a.className = 'lb-ast';
        a.style.top = (8 + Math.random() * 70) + '%';
        a.style.setProperty('--ad', (9 + Math.random() * 9).toFixed(1) + 's');
        a.style.setProperty('--adel', (Math.random() * 9).toFixed(1) + 's');
        const sc = (0.55 + Math.random() * 0.9).toFixed(2);
        a.style.transform = 'scale(' + sc + ')';
        field.appendChild(a);
      }
      wrapEl.prepend(field);
    }
  }
...
function disableCaseBattleEntries() {
  document.querySelectorAll('.navrail-item[href*="case-battles"], .navrail-item[href*="create-battle"], .tile.t-battles, .games-nav a[href*="case-battles"]').forEach((el) => {
    el.classList.add('entry-off');
    el.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); toast('Case Battles are temporarily disabled.'); }, true);
  });
}
...
function renderNav() {
  ...
  if (currentUser) {
    ...
    host.innerHTML = `
      <a class="race-chip" href="#/leaderboard" title="Weekly Race">
        <model-viewer class="race-coin" src="models/robux.glb" alt="" auto-rotate rotation-per-second="20deg"
          camera-orbit="0deg 75deg 100%" field-of-view="30deg" interaction-prompt="none"
          disable-zoom shadow-intensity="0" exposure="1.2"></model-viewer>
        <b>100K RACE</b>
      </a>
      <div class="user-menu">
...
async function initPage() {
  ...
  renderPv();
  renderFeatured();
  initSparkles(document.querySelector('.dash-promo'), 14);
  initDpModelCycle();
  ...
  if (bSlider) bSlider.addEventListener('input', () => {
    ...
    if (badge) badge.textContent = 'Pay ' + borrowPct + '%';
    if (desc) desc.textContent = 'Borrow ' + borrowPct + '% - win only ' + (100 - borrowPct) + '% of the pot';
    if (borrowOn) renderPv();
  });
...
const AudioFX = {
  ...
  blip(f = 1200, d = 0.04, g = 0.035, type = 'square') {
    if (!this.on) return;
    try {
      const c = this.ensure();
  ...
  tick() { this.blip(1400 + Math.random() * 250, 0.018, 0.022, 'triangle'); },
  land() { this.blip(660, 0.07, 0.05, 'sine'); this.blip(990, 0.14, 0.045, 'sine'); },
  win() { [523, 659, 784, 1046, 1318].forEach((f, i) => setTimeout(() => this.blip(f, 0.14, 0.045, 'triangle'), i * 85)); },
  gold() { [1318, 1568, 2093, 2637].forEach((f, i) => setTimeout(() => this.blip(f, 0.12, 0.04, 'sawtooth'), i * 70)); },
  lose() { this.blip(220, 0.2, 0.045, 'sawtooth'); this.blip(140, 0.3, 0.04, 'sawtooth'); },
  click() { this.blip(900 + Math.random() * 80, 0.02, 0.018, 'sine'); },
  add() { this.blip(880, 0.035, 0.03, 'sine'); this.blip(1320, 0.05, 0.028, 'sine'); },
  remove() { this.blip(420, 0.05, 0.026, 'sine'); },
  confirm() { [587, 784, 1175].forEach((f, i) => setTimeout(() => this.blip(f, 0.1, 0.035, 'triangle'), i * 80)); },
  denied() { this.blip(180, 0.14, 0.04, 'sawtooth'); },
};
...
function resolveBattleHeadless(b) {
  ...
  let winIdx;
  if (b.type === 'crazy') {
    // crazy: the LOWEST total value takes the pot — flip of the jackpot
    let bi = 0;
    totals.forEach(function (t, i) { if (t < totals[bi]) bi = i; });
    winIdx = [bi];
  } else if (b.type === 'jackpot') {
  ...
  let share = Math.round((pot / winIdx.length) * 100) / 100;
  if (b.borrow && b.borrow > 0 && b.borrow < 100) share = Math.round(share * (100 - b.borrow)) / 100;
...
function renderLive(b) {
  ...
  const goldRound = (done) => {
    ...
    if (strip) {
      ...
      // one smooth continuous glide — no overshoot, no landing jump
      strip.style.transition = 'transform 5.8s cubic-bezier(.09,.79,.15,1)';
      strip.style.transform = 'translateY(' + target + 'px)';
      // golden aura + drifting particles behind the reel while it spins
      const colEl = document.getElementById('pcol' + gi);
      if (colEl && !colEl.querySelector('.gold-fx')) {
        const fx = document.createElement('div');
        fx.className = 'gold-fx';
        for (let p = 0; p < 14; p++) {
          const dot = document.createElement('i');
          dot.style.left = (Math.random() * 90 + 5) + '%';
          dot.style.setProperty('--gd', (2.5 + Math.random() * 3).toFixed(2) + 's');
          dot.style.setProperty('--gdelay', (Math.random() * 2).toFixed(2) + 's');
          fx.appendChild(dot);
        }
        colEl.appendChild(fx);
      }
    }
    ...
    liveTimers.push(setTimeout(function () {
      ...
      const fxEl = colEl && colEl.querySelector('.gold-fx');
      if (fxEl) fxEl.remove();
  ...
  const finish = function (skipGold) {
    ...
    if (b.type === 'jackpot' || b.type === 'crazy') {
      // jackpot: odds = your share of the pot. crazy: odds flip — the SMALLER
      // your pull, the HEAVIER your slice of the wheel.
      let weights = totals.map(function (t) { return t > 0 ? t : 0.0001; });
      if (b.type === 'crazy') {
        const maxT = Math.max.apply(null, totals) || 1;
        weights = totals.map(function (t) { return Math.max(0.0001, maxT - t + 1); });
      }
      const totalW = weights.reduce(function (a, v) { return a + v; }, 0);
      let r = rng() * totalW;
      let best = weights.length - 1;
  ...
  function finalizeBattle(winIdx) {
    ...
    Api.completeBattle(b.id);
    // weekly race: every real player's buy-in counts toward their wager
    players.forEach(function (p) {
      if (p && !p.you && p.n) return; // bots skip
    });
    b.teams.forEach(function (team) {
      team.forEach(function (pl) {
        if (pl && pl.you && currentUser && currentUser.name) {
          const buyIn = b.cases.reduce(function (s2, k) { return s2 + (CASE_TYPES[k] ? CASE_TYPES[k].price : 0); }, 0);
          Api.addWager(currentUser.name, buyIn).then(function (top) { if (top) window.__lbTop = top; });
        }
      });
    });
...
(function () {
  ...
  if (CASE_BATTLES_DISABLED && (h.startsWith('#/case-battles') || h.startsWith('#/create-battle') || h.startsWith('#/battle/'))) {
    location.hash = '#/home';
    toast('Case Battles are temporarily disabled.');
    return;
  }
    let key = null;
